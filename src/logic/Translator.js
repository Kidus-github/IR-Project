...
const IR_transliteration_lookup_table = {
  "": "X",
  ሀ: "he",
  ሁ: "hu",
  ሂ: "hi",
...
/**
 *@deprecated Use {@link IR_transliterate} function instead.
 */
export function sera_transliterate(word, lang) {
  let trans_word = "";
  ...
  if (lang === "am") {
    ...
    tokens.forEach((letter) => {
      if (transliteration_lookup_table[letter] !== undefined) {
        trans_word += transliteration_lookup_table[letter];
      }
    });
  } else if (lang === "en") {
    ...
    if (tokens) {
      tokens.forEach((letter) => {
        let en_letter = Object.keys(transliteration_lookup_table).find(
...
export function IR_transliterate(word, lang) {
  let trans_word = "";

  if (lang === "am") {
    let tokens = word.split("");
    tokens.forEach((letter) => {
      if (IR_transliteration_lookup_table[letter] !== undefined) {
        trans_word += IR_transliteration_lookup_table[letter];
      }
    });
  } else if (lang === "en") {
    let tokens = word.match(/.{1,2}/g);
    ...
    tokens.forEach((letter) => {
      if (/[^aeiou][aeiou]/i.test(letter)) {
        ...
        if (/[W][a]/g.test(letter)) {
          am_letter = Object.keys(IR_transliteration_lookup_table).find(
            (key) =>
              IR_transliteration_lookup_table[key] === letter.toLowerCase()
          );
        } else {
          am_letter = Object.keys(IR_transliteration_lookup_table).find(
            (key) => IR_transliteration_lookup_table[key] === letter
          );
        }

        if (!am_letter) {
          console.error("No matching letter found");
        }
        ...
        ltrs.forEach((ltr) => {
          am_letter += Object.keys(IR_transliteration_lookup_table).find(
            (key) => IR_transliteration_lookup_table[key] === ltr
          );
        });
